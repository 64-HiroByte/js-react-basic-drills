...
解答ファイル: `src/drills/16_react-map-rendering-pitfalls.jsx`

---

## 目的

map・key・再レンダリングの関係を理解し、UI バグを未然に防ぐ

> **関連ドリル**:
> - stale closure の詳細は **19_js-usememo-usecallback-drill.md** を参照
> - map / filter の基礎は **07_js-map-filter-drill.md** を参照
...
問題4のコードを、正しく合計値を state に入れる形に修正してください。
...
setCount(total); // 1回だけ setState
```

**ポイント**: reduce で先に計算を完了させてから、1回だけ setState を呼ぶ。
...
問題6のコードを、正しく count がインクリメントされるように修正してください。
...
React が渡す**最新の state** を参照できる。

### なぜ関数形式で解決するか

```jsx
// NG: count を直接参照（クロージャで古い値が固定される）
setCount(count + 1);

// OK: prev を使う（React が最新値を渡してくれる）
